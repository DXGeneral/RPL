<?php
	...
	if (isset($_GET["nim"])) {
		if (isset($_GET["id"])) {
			...
			$result = mysqli_query($conn, "SELECT Keaktifan FROM keaktifan_anggota WHERE NIM = $nim  AND ID = $id");
			$row = mysqli_fetch_assoc($result);
			$aktif = $row['Keaktifan'];
		}
	}
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">Ubah Data Keaktifan
                    <small>HIMASTI</small>
                </h1>
                <ol class="breadcrumb">
            ...
            <div class="col-md-9">
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <form action="ubahKeaktifan_diSQL.php?nim=<?php echo "$nim"?>&id=<?php echo "$id"?>" method="POST">
                            <div class="control-group form-group">
                                <div class="controls">
                                    <label>Keaktifan</label>
                                    <input type="text" class="form-control" name="keaktifan" value=<?php echo "$aktif" ?>
                                </div>
                            </div>
                                                                        
                            <div id="success"></div>
                            <!-- For success/fail messages -->
                            <button onclick="alert('Success!')" type="submit" class="btn btn-primary">Ubah</button>
                        </form>
                    </div>
                </div>
